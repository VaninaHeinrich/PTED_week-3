...
```{r}
caro2 <- caro_filter2 %>%ggplot(aes(E, N), group_by(caro_filter2$segment_id))  + geom_path(color = caro_filter2$segment_id) + geom_point(color = caro_filter2$segment_id) + coord_fixed() + theme(legend.position = "right") + ggtitle("Long segments (removed seg < 5)")
caro2
```
